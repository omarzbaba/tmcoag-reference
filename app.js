...
function renderReactions() {
  head("⚠️ Transfusion reactions & biovigilance", "NHSN Hemovigilance Module adverse-reaction definitions (AABB). Tap a reaction for its case definition — Definitive / Probable / Possible. Severity & imputability apply to every reaction (reference below).");
  const r = KB.reactions || {};
  // severity + imputability are reference scales that apply to EVERY reaction (as in
  // the biovigilance document), not repeated per entity.
  content.append(el("details", { class: "card legend-card" },
...
function reactionDetail(rx) {
  ...
  // ---- the biovigilance-document box: name → definition → Definitive/Probable/Possible ----
  const box = el("section", { class: "card rx-doc" });
  box.append(el("div", { class: "rx-head" }, el("h2", { class: "rx-name" }, rx.name),
    rx.acuity ? el("span", { class: "badge pill" }, rx.acuity) : null,
    rx.frequency ? el("span", { class: "badge grade" }, rx.frequency) : null));
  if (rx.definition) box.append(el("p", { class: "rx-def" }, rx.definition));
